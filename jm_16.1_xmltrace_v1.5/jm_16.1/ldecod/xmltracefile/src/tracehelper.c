...
{
//disabling the generation of Motion Vectors trace
/*
	int j0, i0, j, i, list;
...
{
	int i, j, k,l;
	int validBlocks[4][4];//Blocks having DCT-AC coefficients
	int dctCoeffs8_8[8][8]; //DCT coefficients of a 8x8 core transformed blocks
	int dctCoeffs4_4[4][4];//DCT coefficients of 4x4 core transformed blocks
	int acc;
	xml_write_start_element("Coeffs");
		//Case where a 8x8 dct core transform is used
		if(currMB->luma_transform_size_8x8_flag)
		{
			...
			for(i=0; i<2; i++)
			{
				for(j=0; j<2; j++)
				{
					//copying dct coefficients of the current block
					for(k=0; k<8; k++)			
					{
						for(l=0; l<8; l++)
						{
							//(&currSlice->mb_rres[0][0])[i][j]
							dctCoeffs8_8[k][l] = (&currSlice->mb_rres[0][0])[i*8 + k][j*8 + l];
						}
					}
					dctCoeffs8_8[0][0] = 0;
					acc = 0;
					for(k=0; k<8; k++)			
					{
						...
						validBlocks[i*2][j*2] = acc;
						validBlocks[i*2][j*2 + 1] = acc;

						validBlocks[i*2 + 1][j*2] = acc;
						validBlocks[i*2 + 1][j*2 + 1] = acc;
		...
		{	//Case where a 4x4 core transform is used
			//Luma
			for(i=0; i<4; i++)
			{
				for(j=0; j<4; j++)
				{
					...
					for(k=0; k<4; k++)			
					{
						for(l=0; l<4; l++)
						{
							dctCoeffs4_4[k][l] = currSlice->cof[0][i*4 + k][j*4 + l];//currentSlice->(&currSlice->mb_rres[0][0])[i*4 + k][j*4 + l];
						}
					}
					dctCoeffs4_4[0][0] = 0;
					acc = 0;
...
{
	//disabling the generation of NAL infos
	/*
	char typestring[256];
