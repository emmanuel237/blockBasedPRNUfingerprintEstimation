function [RP,LP] = getFingerprint(videoPath,frameIndices,videoParams,sigma) 
% -------------------------------------------------------------------------
...
% for direct, indirect, special, incidental, or consequevideo fvideo file contain extracted with the function videoToMatile contain extracted with the function videoToMatntial damages,
...
% -------------------------------------------------------------------------
%Modified by Emmanuel Kiegaing, Uludag university 2017
%the input argument is no longer a list of image names but the path of the video
%from which the fingerprint is to be extracted. These images have been
%previously saved to the disk by the function loadVideoToDisk
%the output argument ImagesinRP have been removes since it represents the
%image files effectively used for fingerprint estimation and is now
%neaningless 
